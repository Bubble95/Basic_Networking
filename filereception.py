...
def connect():
    ...
    try:
        ...
        #fileObject = open(file['name']+' copy', 'w')
...
#buttonOpen = tk.Button(m, text='open',width=25)
#buttonOpen.grid(row = 0, column = 3,padx = 4, pady = 4)


buttonSend = tk.Button(m, text='connect',width=25,command= connect)
buttonSend.grid(row = 0, column = 3,padx = 4, pady = 4)

buttonDestroy = tk.Button(m, text='close',width=25,command=m.destroy)
buttonDestroy.grid(row = 1, column = 3,padx = 4, pady = 4)
